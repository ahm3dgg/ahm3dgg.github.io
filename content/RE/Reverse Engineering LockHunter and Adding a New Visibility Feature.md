...
File Backed Sections are memory mapped files, they can be data files or executables, when its an executable we call it an image backed section, they are called File Backed because they refer to an actual file on disk something like `travis_scoot_top_hits.txt` or `don_toliver_scareware.exe`, for memory mapped files that are data files, when they are dirty or have been modified the content is reflected to the file on disk, that's not the case for image sections.

When you create an Image Section, the file gets locked and the content written in memory doesn't get reflected in the file, to create an image section you have to first open a handle to the file, that gives you a `File` handle, then create the section that gives you another handle the `Section` handle, then you can map the section to make the file content from your process's virtual address space, Let's look at some code I wrote using the NT API
...
- Mapping the sections using `NtMapViewOfSection` to make it available in my virtual address space. 

- I also introduced some `getchar()`s, just to see what will happen when I close each one at a time, that will help when trying to understand LockHunter's behaviour
...
Okay now I am able to rename the file for instance, let's try deleting it

![[Pasted image 20260226113638.png]]

Oh oh I can't, but windows tells us the process that is using the file, so its a nice thing.
...
I think looking at the LockHunter's folder would be a good point to start, since we can start identifying the files it uses.
...
So Now we have some information, maybe we can fire up IDA Pro, and looks if there are any calls to this function, I started by examining the Import Address Table, and Yes I found it

![[Pasted image 20260226122944.png]]

In usermode `Nt` and `Zw` are equivalent, here really `ZwQuerySystemInformation` maps to `NtQuerySystemInformation` in NTDLL's export table.

The function is calling from one place, which I named `QuerySystemInformation`
...
To understand what its doing, we have to understand how this function works, this function works by providing it a pointer to the buffer that it will fill up the data in, and in case the buffer is not large enough a `STATUS_INFO_LENGTH_MISMATCH` error is returned, so the programmer will usually try to allocate in a loop until a `STATUS_SUCCESS` is returned, here its doubling the size every time it the function fails.

Doing XREFs on this function, we can see its called from a function The Programmer named `ScanForLockingHandles`, I mean I love it when programmers leave there logging messages :D

![[Pasted image 20260226123842.png]]

![[Pasted image 20260226123931.png]]

Let's focus on the relative parts of this function, the first relevant thing the function does is obtaining the file object type index, each windows executive object has some id that identifies it, this id however changes between different versions of windows, LockHunter here does a very neat trick in order to obtain the File Object Type Index
...
as I said above, the function calls into a function I named `QueryFileObjectInfo`, this, understanding this function requires looking at the driver, however in short the function takes in a pointer to the handle entry retrieved by `NtQuerySystemInformation`, and reads the kernel object address from the `Object` field, and copies it in the IOCT input buffer, it then sends an IOCTL to the kernel driver `USRFindHandle64.sys`
...
so following the x64 calling convention, `dwIoControlCode` should be at rdx, which in this case it has the value `0x9C402400`, we will look at hook IOCTLs are encoded in the next section. 

At this point we have the following information 

- It uses `NtQuerySystemInformation` to query all system handles
- It only cares about handles referring to Files
- It sends an IOCTL with as input equal to the kernel object address that is referred to by a File Handle
...
DriverEntry will set the the IRP Handlers, in the DriverObject Dispatch Table, so when the usermode application calls into the driver, the kernel knows which functions to call, it also registers an unload routines, and create a device object, here we can see this path `\\Device\\USR_Find_Handle0`, this is the path that will be used by the usermode application, the usermode application typically will open a file handle to the device object, referred to by this path, and then pass that handle to `DeviceIoControl` or `NtDeviceIoControlFile`
...
So now going back to `UsrQueryFileObjectInfo` function, since we now know that its using Buffered I/O and we also know from reversing the usermode application that it sends the kernel object address of a File object, so we can starting changing types in IDA, so the decompilation looks, cleaner, for example we can see its reading a pointer from the system buffer, which as I said above its located at `Irp->Associated.SystemBuffer`, I have set this to PFILE_OBJECT. 

```cpp
FileObject = *Irp->AssociatedIrp.SystemBuffer;
```
...
Now Enough Reverse Engineering, I am sure you may have already been to attack the program without digging that deep, and in fact I did so, but I decided to make the blog long and informative :)
### How LockHunter Closes Handles ?

Again, as with the question of how to query system handles, I actually didn't know how would you close a handle into a remote process, and I started reading through SystemInformer code, and also found this [Blog by Pavel Yosifovich](https://scorpiosoftware.net/2020/03/15/how-can-i-close-a-handle-in-another-process/)
...
`NtDuplicateObject`, will duplicate a handle from one process into another process, the handle to be duplicated is at `SourceHandle`, `NtDuplicate` has an interesting flag called `DUPLICATE_CLOSE_SOURCE` and according to ntdoc
...
We can see that the handle value is `0xA8`, that's equal to the one we can see in rdx, after returning from `NtDuplicateObject`, the handle should be closed and LockHunter will present its Green Checkmark, but we for sure know that the file is not unlocked yet, so let's fix this :D
### Planning The Attack
...
So enough yapping, let me show how I went about implementing implementing that feature, So we know that the usermode application sends the kernel object address, and with our usermode hook, we are now sending both File Objects and Section Objects, here is the modified version of the IRP Handler
...
First we check to see the object type using an undocumented function `ObGetObjectType` this function is exported however, its not documented in the WDK, every executive object in windows has a type, which is really an object it self, called `OBJECT_TYPE`, `OBJECT_TYPE` holds static information that are same for all instances of a specific object, it also links objects of same type together, this was used to save memory so that you don't have to embed a static information, in every object object's header.
...
Also, There is one case where it will return null, this is when the section is page-file backed, so it doesn't refer to an actual file on disk, so I make sure to check for that.

Here The code I use to grab `MmGetFileObjectForSection`, you can find the patterns as well as the full source code in the GitHub repo associated with this blog 
